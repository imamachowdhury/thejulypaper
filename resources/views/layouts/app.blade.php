            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-10">
                    <a href="{{ url('/') }}" class="inline-block">
                        <span class="text-3xl md:text-5xl font-black tracking-tighter text-white">দ্যা জুলাই <span class="text-white/80">পেপার</span></span>
                    </a>
                </div>

                <!-- Row 1: Primary Categories -->
                <div class="flex flex-wrap gap-x-10 gap-y-4 mb-10 text-base md:text-lg font-bold">
                    <a href="{{ url('/') }}" class="hover:text-white/80 transition-colors">প্রচ্ছদ</a>
                    @php 
                        $primaryMenus = \App\Models\MenuLink::where('location', 'primary')->orderBy('sort_order')->get();
                    @endphp
                    @foreach($primaryMenus as $menu)
                        <a href="{{ $menu->computed_url }}" class="hover:text-white/80 transition-colors pa-headline">{{ $menu->label }}</a>
                    @endforeach
                </div>
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-8">
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-2 text-sm font-medium text-white/90">
                        <span>&copy; {{ toBangla(date('Y')) }} দ্যা জুলাই পেপার</span>
                        
                        @php 
                            $footerLinks = \App\Models\MenuLink::where('location', 'footer')->orderBy('sort_order')->get();
                        @endphp
                        @foreach($footerLinks as $link)
                            <span class="hidden md:inline text-white/30 text-lg">|</span>
                            <a href="{{ $link->computed_url }}" class="hover:text-white transition-colors">{{ $link->label }}</a>
                        @endforeach
                    </div>

                    <!-- Right: Social Media -->
                    <div class="flex items-center space-x-6">
                        <span class="text-xs font-bold uppercase tracking-widest text-white/80">আমাদের অনুসরণ করুন:</span>
                        <div class="flex items-center space-x-4">
                            <!-- Facebook -->
                            <a href="#" class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white text-pa-red transition-all shadow-sm">
